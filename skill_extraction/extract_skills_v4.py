"""
extract_skills_v4.py — Generate v4 skill profiles (single-version, 3-file design).

Design (EXTRACTION_PROMPTS_v4.md):
  - 3 files per persona: background.txt, decision_profile.txt, evaluation_profile.txt
  - Single version — no v1/v2/v3 split
  - 3 API calls per persona, one per file
  - Module allocation is strict (see EXTRACTION_PROMPTS_v4.md for full spec)
...
Usage (from Digital-Twin-Simulation/):
    python skill_extraction/extract_skills_v4.py --pid 1
    python skill_extraction/extract_skills_v4.py --pid 1 --force
    python skill_extraction/extract_skills_v4.py --pid 1 --file background
"""
...
OUTPUT_BASE = Path("text_simulation/skills_v4")

# ── Token budgets (from EXTRACTION_PROMPTS_v4.md) ────────────────────────────
MAX_TOKENS = {
    "background":         2000,
    "decision_profile":   1800,
    "evaluation_profile": 2200,
}
...
- A.4.5: Overconfidence — 1 item (predicted own score minus actual score)
- A.4.6: Overplacement — 1 item (own prediction minus average prediction)
...
### Loss Aversion
Lambda coefficient: [X] (1.0 = symmetric weighting; >1.0 = losses weighted more
...
Receiver — report the return for each of the 5 possible received amounts:
  - Received $15 (sent $5 x 3): returned $[X] ([X]%)
  - Received $12 (sent $4 x 3): returned $[X] ([X]%)
  - Received $9  (sent $3 x 3): returned $[X] ([X]%)
  - Received $6  (sent $2 x 3): returned $[X] ([X]%)
  - Received $3  (sent $1 x 3): returned $[X] ([X]%)
...
FILES = [
    # (file_key, filename, prompt, marker)
    ("background",         "background.txt",         PROMPT_BACKGROUND, "---BACKGROUND---"),
    ("decision_profile",   "decision_profile.txt",   PROMPT_DECISION,   "---DECISION_PROFILE---"),
    ("evaluation_profile", "evaluation_profile.txt", PROMPT_EVALUATION, "---EVALUATION_PROFILE---"),
]
...
def extract_for_persona(pid: str, files_filter: list[str] | None = None, force: bool = False):
    """Extract v4 skill files for a single persona (real-time, not batch)."""
    ...
    for file_key, filename, prompt_template, marker in FILES:
        ...
        if not force and out_path.exists():
            print(f"  [{filename}] exists — skipping (--force to redo)")
            continue

        max_tok = MAX_TOKENS[file_key]
        prompt  = prompt_template.format(transcript=transcript)
        print(f"  [{filename}] calling API (max_tokens={max_tok})...")

        raw     = call_api(client, prompt, max_tok)
        content = parse_section(raw, marker)

        # Retry with doubled tokens if ## Summary is missing
        if "## Summary" not in content:
            doubled = min(max_tok * 2, 4096)
            print(f"  [{filename}] Summary missing — retrying with {doubled} tokens")
            raw     = call_api(client, prompt, doubled)
            content = parse_section(raw, marker)
