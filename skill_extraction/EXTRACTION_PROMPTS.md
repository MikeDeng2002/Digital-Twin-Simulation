# Extraction Prompts — Three-File Design

Nine prompts total: three versions (v1_direct, v2_inferred, v3_maximum) for each
of three files (background, decision_procedure, evaluation_profile). Drop these
into `extract_skills.py`. Each prompt takes the full persona transcript where
`{transcript}` appears and returns output with a single structural marker.
...
- Decision procedure prompts: `---DECISION_PROCEDURE---`
- Evaluation profile prompts: `---EVALUATION_PROFILE---`
...
- **`background.txt`** — what the person knows, believes, and values; demographics; personality; inferred domain familiarity (the "lived knowledge" signal)
- **`decision_procedure.txt`** — how she reasons under uncertainty; cognitive style; bias susceptibility (epistemic)
- **`evaluation_profile.txt`** — what she wants; willingness-to-pay; preference structure (motivational)

The three files map to the classical belief–desire decomposition plus reasoning style, with no redundancy.
...
## Prompt for `background.txt` — v1_direct
...
## Prompt for `background.txt` — v2_inferred
...
## Prompt for `background.txt` — v3_maximum
...
# DECISION_PROCEDURE PROMPTS

---

## Prompt for `decision_procedure.txt` — v1_direct
...
## Prompt for `decision_procedure.txt` — v2_inferred
...
- Self-rated as "generous" but offers selfish splits in the Ultimatum Game
- Low self-reported Neuroticism but high somatic anxiety symptoms
- Self-rated conscientious but self-describes as "overwhelmed" and
  procrastinating in free-response items
...
performance. Name specific error patterns (e.g., "answered 27 cubic feet on
the hole-digging problem — classic fast-intuitive trap answer"). Cross-
reference with Conscientiousness from Big Five where relevant.
...
satisficing on some domains but maximizing on others (e.g., satisficer on
purchases but maximizer on social comparison), name the split with evidence.
...
## Prompt for `decision_procedure.txt` — v3_maximum
...
2. COGNITIVE ERROR PATTERNS MATTER MORE THAN TOTAL SCORES — a person who
   answered "27" on the 3x3x3 hole question reveals cognitive miserliness
   even if their overall CRT-equivalent score is decent. Name specific
   error types.

3. SOMATIC-VS-SELF-REPORT CALIBRATION — if Beck-style anxiety symptoms
   show moderate-to-severe loading but Big Five Neuroticism is low, note
   this as a self-report reliability issue that affects all other self-
   reports.

4. FREE-RESPONSE TEXT IS HIGH-SIGNAL — aspirational-self and actual-self
   descriptions often reveal self-awareness that scales miss. Quote or
   paraphrase them when they illuminate reasoning style.

5. ACKNOWLEDGE PREDICTIVE LIMITS — per Stanovich & West (2008), most H&B
   biases (framing, anchoring, base-rate, conjunction, sunk cost, myside)
   are NOT reliably correlated with cognitive ability. For these, express
   any prediction with appropriate uncertainty. Do NOT produce confident
   bias predictions the literature doesn't support.
...
lottery/risk tasks, cognitive tests (syllogisms, Wason, analogies, CRT-
style items), word association chain, social desirability, and
demographics.
...
response self-descriptions to personality items. Compare self-reported
Neuroticism to somatic symptoms. Use social desirability score to weight
self-reports globally — if social desirability is high, all self-reports
should be discounted and behavioral data weighted more heavily.
...
Combined with the person's own views in background.txt, this predicts
their false-consensus errors.
...
- Risk-benefit nonseparability (affect heuristic — also predictable from
  emotional reactivity signals)
...
observed, somatic-vs-self-report calibration. This weighting applies to
all scales and should be used when simulation tasks rely on the persona's
stated preferences.
...
# EVALUATION_PROFILE PROMPTS

---

## Prompt for `evaluation_profile.txt` — v1_direct
...
## Prompt for `evaluation_profile.txt` — v2_inferred
...
## Prompt for `evaluation_profile.txt` — v3_maximum
...
- Value-laden policy items (ban agreement, omission bias on vaccines)
...
4. STATED-VS-REVEALED GAP IN SOCIAL PREFERENCES — if the person rates
   ...
   games, the behavioral data wins for predicting actual allocation. Use
   stated values only for context.
...
family-generosity, name this as an in-group cooperator profile. This has
specific implications for allocation tasks involving unknown vs. known
recipients.

## Policy and value-laden preferences
For any policy items in the transcript (bans, vaccines, immigration,
healthcare), state the person's direct preferences. These predict myside
bias direction on those items and answer choices on value-laden cognitive
tasks.
...
and any behavioral evidence. Do not commit to specific per-product
purchase decisions — describe patterns.

## Willingness-to-pay calibration
State the likely price ceiling for common grocery categories based on
income and minimalism. If the person is strongly minimalist, the ceiling
is lower than income alone would predict. Give rough dollar ranges where
reasonable.
...
acquiescence patterns. This weighting tells the downstream simulation how
much to lean on the values scale versus the behavioral game data when they
conflict.
...
3 files × 3 versions = **9 API calls per persona**, same as the original CLAUDE.md design.

If API cost matters, you can combine all three files for one version into a single prompt returning three marked sections (same as the original design). This reduces to 3 calls per persona. Recommended: run the 9-call version first for quality, then optimize to combined prompts once the output is stable.
...
decision   = parse_section(response, "---DECISION_PROCEDURE---")
evaluation = parse_section(response, "---EVALUATION_PROFILE---")
```

## Token budget

- v1 prompts: `max_tokens=1500` is sufficient
- v2 prompts: `max_tokens=2000` recommended (longer due to reasoning-chain labels)
- v3 prompts: `max_tokens=2500` minimum (more sections, more detail)

Retry with doubled `max_tokens` if the `## Summary` section is missing from the output — that's the most common truncation symptom.
...
Three files per version, nine files total per persona. No `tools.txt`.
