"""
create_skills_v4_bg_ep_inputs.py — Build combined LLM prompt files from v4 skill
profiles using ONLY background + evaluation_profile (omits decision_profile).

Reads two skill files from text_simulation/skills_v4/pid_{pid}/ and combines each
with the per-persona survey question file from text_simulation/text_questions/
pid_{pid}.txt to produce text_simulation/text_simulation_input_skills_v4_bg_ep/
pid_{pid}_prompt.txt.

Persona-safety: every pid is matched by EXACT filename (pid_{N}.txt) and the
survey file is required to exist. A previous version of this pipeline had a
mis-alignment bug; here we assert exact-match to prevent recurrence.

Usage (from Digital-Twin-Simulation/):
    python text_simulation/create_skills_v4_bg_ep_inputs.py
    python text_simulation/create_skills_v4_bg_ep_inputs.py --pids 1,2,3
"""
...
OUTPUT_DIR    = Path("text_simulation/text_simulation_input_skills_v4_bg_ep")
...
def main():
    ...
    if args.pids:
        ...
        print(f"Found {len(pids)} personas in {SKILLS_V4_DIR}")
