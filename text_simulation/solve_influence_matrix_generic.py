...
Usage: python solve_influence_matrix_generic.py <topology_name>
...
# ============================================================
if len(sys.argv) < 2:
    print("Usage: python solve_influence_matrix_generic.py <topology_name>")
...
print(f"\n{'='*70}")
print(f"Solving influence matrix for: {cfg['label']}")
print(f"{'='*70}")
...
for i in range(n_agents):
    ...
    for j in range(n_free):
        def make_upper(j=j):
            def fun(z): return z[n_free + j] - z[j]
            def jac(z):
                g = np.zeros(2 * n_free); g[j] = -1.0; g[n_free + j] = 1.0; return g
...
plt.suptitle(f"Predicted vs Actual Sentiment — {cfg['label']}", fontsize=14, fontweight="bold")
plt.tight_layout()
